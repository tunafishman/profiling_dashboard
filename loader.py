...
class RedShiftLoader():
	...
	def Query(self, query):
		...
		rs_query = """select partition1 as network, partition2 as geo, partition3 as url_domain, partition4 as size, count(*) as count, class, max(median_speed) as speed, max(median_fbu) as fbu, max(median_dcu) as dcu, max(extra0) as size_25, max(extra1) as size_50, max(extra2) as size_75 from (select network as partition1, geo as partition2, url_domain as partition3, case when size < 10000 then 'Small' when size < 30000 then 'Medium' when size < 50000 then 'Medium+' when size < 200000 then 'Large' else 'X-Large' end as partition4, class, median(speed) over (partition by network, geo, url_domain, case when size < 10000 then 'Small' when size < 30000 then 'Medium' when size < 50000 then 'Medium+' when size < 200000 then 'Large' else 'X-Large' end, class) as median_speed, median(fbu) over (partition by network, geo, url_domain, case when size < 10000 then 'Small' when size < 30000 then 'Medium' when size < 50000 then 'Medium+' when size < 200000 then 'Large' else 'X-Large' end, class) as median_fbu, median(dcu) over (partition by network, geo, url_domain, case when size < 10000 then 'Small' when size < 30000 then 'Medium' when size < 50000 then 'Medium+' when size < 200000 then 'Large' else 'X-Large' end, class) as median_dcu, percentile_cont(0.25) within group (order by size) over (partition by network, geo, url_domain, case when size < 10000 then 'Small' when size < 30000 then 'Medium' when size < 50000 then 'Medium+' when size < 200000 then 'Large' else 'X-Large' end, class) as extra0, median(size) over (partition by network, geo, url_domain, case when size < 10000 then 'Small' when size < 30000 then 'Medium' when size < 50000 then 'Medium+' when size < 200000 then 'Large' else 'X-Large' end, class) as extra1, percentile_cont(0.75) within group (order by size) over (partition by network, geo, url_domain, case when size < 10000 then 'Small' when size < 30000 then 'Medium' when size < 50000 then 'Medium+' when size < 200000 then 'Large' else 'X-Large' end, class) as extra2 from tplog where datetime > '2015-10-31' and cid = 3521 and network is not null  and geo is not null  and url_domain is not null  and size is not null ) as nested  group by network, geo, url_domain, size, class order by network, geo, url_domain, size, class limit 50000;"""
